...
public class SettingController extends AdminBaseController {
    ...
    @Autowired
    public SettingController(ISms sms, IEmail email, PlatformSettingService platformSettingService, PlatformLogService platformLogService) {
        this.sms = sms;
        this.email = email;
     ...
     * 系统日志
     */
    @GetMapping("/platformLog.html")
    public String platformLog(int page, int size,
        ...
        model.addAttribute("page",  platformLogService.page(page, size, column,order , field, value,beginDate, endDate));

        return "platform/platformLog";
    }
