...
public class UserBusinessRepositoryImpl implements UserBusinessRepository {
    ...
    public Page<Projector> pageOfProjector(Pageable pageable) {
        String querySql="SELECT `userId`, `name`, `company`, `title`, ( SELECT count(1) FROM t_user_visitor visitor WHERE visitor.visitorUserId = business.userId ) AS `visitorTotals`, ( SELECT count(1) FROM t_user_item_accept accept WHERE accept.userId = business.userId ) AS `receiveItemTotals` FROM t_user_business business WHERE auditStatus = 'PASS_AUDIT'";
    ...
    public ProjectorIntro projectorIntro(long userId) {
        String querySql="SELECT id, `name`, company, title, ( SELECT COUNT(1) FROM t_user_attention attention WHERE attention.userId = business.userId ) AS `attentionTotals`, ( SELECT COUNT(1) FROM t_user_attention attention WHERE attention.attentionUserId = business.userId ) AS `beAttentionTotals`, ( SELECT COUNT(1) FROM t_user_contact contact WHERE contact.userId = business.userId ) AS `contactTotals`, ( SELECT COUNT(1) FROM t_user_contact contact WHERE contact.contactUserId = business.userId ) AS `beContactTotals`, ( SELECT COUNT(1) FROM t_user_visitor visitor WHERE visitor.visitorUserId = business.userId ) AS `visitorTotals`, ( SELECT COUNT(1) FROM t_user_item_accept accept WHERE accept.userId = business.userId ) AS `acceptItemTotals` FROM t_user_business business WHERE business.userId = :userId";
